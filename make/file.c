...
void
vprint_prereqs (const struct dep *deps)
{
  const struct dep *ood = 0;
  ...
  for (; deps != 0; deps = deps->next)
    if (! deps->ignore_mtime)
      fprintf (debugfp, "%s ", dep_name (deps));
    else if (! ood)
      ood = deps;

  /* Print order-only deps, if we have any.  */
	// XXX(wenbin): what's this??
  if (ood)
    {
      vprint (" | %s", dep_name (ood));
      for (ood = ood->next; ood != 0; ood = ood->next)
        if (ood->ignore_mtime)
          fprintf (debugfp, "%s ", dep_name (ood));
    }
}

static void
vprint_file (const void *item)
{
  const struct file *f = item;

  if (!f->is_target)
		return;
  fprintf(debugfp, "DEP---%s---", f->name);
  vprint_prereqs (f->deps);
	fprintf(debugfp, "\n");
}
